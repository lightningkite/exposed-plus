...
fun <TableType : ResultMappingTable<*, InstanceType, KeyType>, KeyType : ForeignKey<TableType>, InstanceType> KeyType.testResolve(): InstanceType {
    return this.table.select(table.matchingKey(this)).first()
        .let { table.convert(it) }
}

fun <TableType : ResultMappingTable<*, InstanceType, KeyType>, KeyType : ForeignKey<TableType>, InstanceType> KeyType.prefill(
    value: InstanceType
) {
    filled = true
    untypedValue = value
